...
import { apiKey, url } from '../../util/constants'
...
export const fetch = payload => {
    ...
    return async (dispatch) => {
        try {
            ...
            const response = await axios.get(`${url}discover/movie?api_key=${apiKey}&language=en-US&sort_by=${sortBy}&include_adult=true&include_video=false&with_genres=${genresList}&with_cast=${castList}&with_keywords=${keywordsList}&primary_release_year=${year}&page=${page}`)
...
export const fetchCast = payload => {
    return async (dispatch) => {
        try {
            ...
            const response = await axios.get(`${url}search/person?api_key=${apiKey}&language=en-US&query=${encodeURIComponent(payload)}&page=1&include_adult=false`)
...
export const fetchKeywords = payload => {
    return async (dispatch) => {
        try {
            ...
            const response = await axios.get(`${url}search/keyword?api_key=${apiKey}&query=${encodeURIComponent(payload)}&page=1`)
